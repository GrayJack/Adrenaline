...
static void isoAlloc(){
	ciso_dec_buf = user_malloc(DAX_BLOCK_SIZE + 64);
	ciso_com_buf = user_malloc(DAX_COMP_BUF + 64);
	g_sector_buffer = user_malloc(SECTOR_SIZE);
	g_CISO_idx_cache = user_malloc(4*CISO_IDX_MAX_ENTRIES);
}

static void isoFree(){
	if (ciso_dec_buf) user_free(ciso_dec_buf);
	if (ciso_com_buf) user_free(ciso_com_buf);
	if (g_sector_buffer) user_free(g_sector_buffer);
	if (g_CISO_idx_cache) user_free(g_CISO_idx_cache);
	ciso_dec_buf = NULL;
	ciso_com_buf = NULL;
...
static void decompress_jiso(void* src, int src_len, void* dst, int dst_len, u32 topbit){
	if (src_len == dst_len) memcpy(dst, src, dst_len); // check for NC area
	else lzo1x_decompress(src, src_len, dst, (uint *)&dst_len, 0); // use lzo
}
...
static int read_compressed_data(void* addr, u32 size, u32 offset) {
	u32 cur_block;
	u32 pos, read_bytes;
	u32 o_offset = offset;
	...
	if (ending_block-g_CISO_cur_idx < CISO_IDX_MAX_ENTRIES-1) { //(ending_block-starting_block+1 < g_cso_idx_cache_num-1){
		...
		read_raw_data(&o_end, sizeof(u32), ending_block*sizeof(u32)+header_size); // read last two offsets
	}
	...
	while(size > 0) {
		...
		if ((void*)c_buf < addr || c_buf+b_size > top_addr) {
...
static int findFile(const char * file, u32 lba, u32 dir_size, u32 is_dir, Iso9660DirectoryRecord *result_record) {
	...
	while (re < dir_size) {
		...
		if (rec->len_dr < rec->len_fi + sizeof(*rec)) {
			log("%s: Corrupted directory record found in %s, LBA %d\n", __func__, g_filename, (int)lba);
		}
		#endif
...
int isoOpen(const char *path) {
	...
	if (memcmp(&g_sector_buffer[1], ISO_STANDARD_ID, sizeof(ISO_STANDARD_ID)-1)) {
		#ifdef DEBUG
		log("%s: vol descriptor not found\n", __func__);
		#endif
		ret = -10;
