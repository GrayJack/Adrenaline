...
SceUID vpbp_open(const char * file, int flags, SceMode mode) {
	...
	if (vpbp == NULL) {
		#ifdef DEBUG
		log("%s: Unknown file %s in vpbp list\n", __func__, file);
		#endif
		unlock();
