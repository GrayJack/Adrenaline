...
static inline u32 get_isocache_magic(void) {
	...
	version = (version << 16) | sctrlHENGetRevisionVersion();
	version += 0xC01DB15D;
...
static int is_inferno_only(const char* filename) {
	//result
	int result = 0;
...
static int get_sfo_section(VirtualPBP *vpbp, u32 remaining, void *data) {
	...
	re = MIN(remaining, virtual_sfo_size() - (vpbp->file_pointer - vpbp->header[2]));
...
static int get_pbp_section(VirtualPBP *vpbp, u32 remaining, int idx, void *data) {
	...
	if(buf == NULL) {
		#ifdef DEBUG
		logmsg("%s: buf(2) cannot allocate\n", __func__);
		#endif
		return -5;
	}
...
int vpbp_init(void) {
	...
	if (g_caches == NULL) {
		g_caches_cnt = 0;
		#ifdef DEBUG
		logmsg("%s: g_cache cannot allocate\n", __func__);
		#endif
		return -27;
	}
...
SceUID vpbp_open(const char * file, int flags, SceMode mode) {
	...
	if (vpbp == NULL) {
		#ifdef DEBUG
		logmsg("%s: Unknown file %s in vpbp list\n", __func__, file);
		#endif
		unlock();
...
int vpbp_loadexec(char * file, struct SceKernelLoadExecVSHParam * param) {
	...
	if (has_pboot){
		...
		// 		apitype = PSP_INIT_APITYPE_UMDEMU_EF2;
		// 	}
		// }
	} else {
