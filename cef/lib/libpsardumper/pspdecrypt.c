...
static TAG_INFO const* GetTagInfo(u32 tagFind) {
	logmsg("Start %s\n", __func__);
	for (int iTag = 0; iTag < sizeof(g_tagInfo)/sizeof(TAG_INFO); iTag++) {
...
static TAG_INFO2 *GetTagInfo2(u32 tagFind) {
	logmsg("Start %s\n", __func__);
	for (int iTag = 0; iTag < sizeof(g_tagInfo2) / sizeof(TAG_INFO2); iTag++) {
...
int pspDecryptIPL3(const u8* pbIn, u8* pbOut, int cbIn) {
	...
	int ret = sceUtilsBufferCopyWithRange(pbOut, cbIn+0x40, pbOut+0x40, cbIn, 1);
	if (ret != 0) {
		Kprintf("mangle#1 returned $%x\n", ret);
		pspSdkSetK1(k1);
		logmsg("End %s\n", __func__);
...
int (*_sceKernelKleDecode)(void *, u32, void *, int) = NULL;
int decompress_kle(void *outbuf, u32 outcapacity, void *inbuf, int is_kdl4) {
	logmsg("Start %s\n", __func__);
	if (NULL == _sceKernelKleDecode) {
		u32 *mod = (u32 *)sceKernelFindModuleByName("sceLoadExec");
		u32 text_addr = *(mod+27);
		_sceKernelKleDecode = (void *)(text_addr+0);
	}

	logmsg("End %s\n", __func__);
	return _sceKernelKleDecode(outbuf, outcapacity, inbuf, is_kdl4);
}

static u32 findFunction(const char *szMod, const char *szLib, u32 nid) {
	logmsg("Start %s\n", __func__);
...
static int _pspDecompress(u32 *arg) {
	...
	if (inbuf[0] == 0x1F && inbuf[1] == 0x8B) {
		...
		if (sceKernelDevkitVersion() >= 0x03080000) {
			...
			retsize = sceKernelLzrcDecode(outbuf, outcapacity, inbuf+4, NULL);
		}

	} else if (memcmp(inbuf, "KL4E", 4) == 0) {
		retsize = decompress_kle(outbuf, outcapacity, inbuf+4, 1);
	} else if (memcmp(inbuf, "KL3E", 4) == 0) {
		retsize = decompress_kle(outbuf, outcapacity, inbuf+4, 0);
	} else {
		retsize = -1;
	}
...
static void DecryptT(u8 *buf, int size, int mode) {
	...
	logmsg("%s: Passed HERE 1\n", __func__);

	for (int i = 0; i < size; i++) {
		logmsg("%s: Passed HERE 2\n", __func__);
		for (int j = 0; j < 8; j++) {
			logmsg("%s: Passed HERE 3\n", __func__);
			m2[7-j] = buf[j];
		}

		logmsg("%s: Passed HERE 4\n", __func__);
		Sce_Paranoia(m1, 0x33333333, (u32 *)&m2[0], (u32 *)&m2[4]);

		for (int j = 0; j < 8; j++) {
			logmsg("%s: Passed HERE 5\n", __func__);
			m1[0x90+j] = m2[7-j] ^ m1[0x80+j];
			m1[0x80+j] = buf[j];
		}

		logmsg("%s: Passed HERE 6\n", __func__);
