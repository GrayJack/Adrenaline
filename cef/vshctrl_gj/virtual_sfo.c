...
unsigned int g_sfo_entries_size = 0;
...
void sfo_load_data_table(void* sfo) {
	...
	if (g_sfo_header->entries_count) {
		g_sfo_data_table.size =
			(g_sfo_entries[g_sfo_header->entries_count - 1].data_offset +
...
void sfo_set_string_param(char *key, char *value) {
	...
	switch (g_sfo_entries[index].param_fmt) {
		...
		case 1024: // Special mode string
			...
			memset(&g_sfo_data_table.content[g_sfo_entries[index].data_offset], 0,
				g_sfo_entries[index].param_max_len);

			// Save new string to data table
			snprintf(&g_sfo_data_table.content[g_sfo_entries[index].data_offset],
				g_sfo_entries[index].param_max_len, "%s", value);
			break;
		case 1028: // Integer
			break;
	}
