...
static PBPEntry g_pbp_entries[8] = {
...
static VirtualPBP *g_vpbps = NULL;
static int g_vpbps_cnt = 0;
static int g_sema = -1;
...
static inline u32 get_isocache_magic(void) {
	u32 version = sctrlSEGetVersion() & 0xF;
...
static int build_vpbp(VirtualPBP *vpbp) {
	...
	for (int i = 0; i < NELEMS(g_pbp_entries); ++i) {
		vpbp->header[i+2] = off;

		if (g_pbp_entries[i].enabled) {
			PBPSection *sec = &vpbp->sects[i];

			ret = isoGetFileInfo(g_pbp_entries[i].name, &sec->size, &sec->lba);
...
static VirtualPBP *vpbp_realloc(VirtualPBP *vpbp, int size) {
	VirtualPBP *v = (VirtualPBP*) oe_realloc(vpbp, size * sizeof(vpbp[0]));

	return v;
}

static int get_header_section(VirtualPBP *vpbp, u32 remaining, void *data) {
	int re = MIN(remaining, vpbp->header[2] - vpbp->file_pointer);
...
int vpbp_read(SceUID fd, void * data, SceSize size) {
	...
	while (remaining > 0) {
		for(int idx=0; idx < NELEMS(g_pbp_entries) - 2; ++idx) {
...
int has_boot_bin(const char *isopath) {
	int k1 = pspSdkSetK1(0);
	int ret = isoOpen(isopath);
...
static void check_controller_input() {
...
int vpbp_loadexec(char * file, struct SceKernelLoadExecVSHParam * param) {
	...
	int has_bootbin = has_boot_bin(vpbp->name);
	...
	if (has_pboot){
		...
		} else if ((g_cfw_config.execute_boot_bin || g_exec_boot_bin) && has_bootbin) {
			param->argp = BOOT_BIN;
		} else {
	...
	//start game image
	int ret = sctrlKernelLoadExecVSHWithApitype(apitype, loadexec_file, param);
...
static int add_fake_dirent(SceIoDirent *dir, int vpbp_idx) {
	VirtualPBP *vpbp = &g_vpbps[vpbp_idx];
