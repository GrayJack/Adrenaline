...
int vpbp_loadexec(char * file, struct SceKernelLoadExecVSHParam * param) {
	...
	if (config.umdmode == MODE_INFERNO) {
		logmsg2("Launching with Inferno Driver\n");
	...
	param->key = "umdemu";
	apitype = SCE_EXEC_APITYPE_DISC_EMU_MS1;
	...
	if (has_pboot){
		...
		apitype = SCE_EXEC_APITYPE_DISC_EMU_MS2;
		...
		// 		apitype = SCE_EXEC_APITYPE_DISC_EMU_EF2;
		// 	}
		// }
	} else {
		...
		// 	if(ret == 0 && 0 == stricmp(devicename, "ef0:")) {
		// 		apitype = SCE_EXEC_APITYPE_DISC_EMU_EF1;
		// 	}
		// }
